...
export default function FunLearningPage() {
  ...
  // Test mode auto-speak effects
  useEffect(() => {
    if (tab === 'shapes' && shapeSub === 'test' && !shapeTestAnswered) {
      speak(shapes[shapeTestIdx % shapes.length].label);
    }
  }, [tab, shapeSub, shapeTestIdx, shapeTestAnswered]);

  useEffect(() => {
    if (tab === 'colors' && colorSub === 'test' && !colorTestAnswered) {
      speakThenSpell(COLORS[colorTestIdx % COLORS.length]);
    }
  }, [tab, colorSub, colorTestIdx, colorTestAnswered]);

  useEffect(() => {
    if (tab === 'd3' && d3Mode === 'test' && !d3TestAnswered) {
      speak(shape3dList[d3TestIdx % shape3dList.length].label);
    }
  }, [tab, d3Mode, d3TestIdx, d3TestAnswered]);

  useEffect(() => {
    if (tab === 'teen' && teenMode === 'test' && !teenTestAnswered) {
      speak(TEEN_WORDS[teenTestIdx % TEEN_WORDS.length].word);
    }
  }, [tab, teenMode, teenTestIdx, teenTestAnswered]);

  useEffect(() => {
    if (tab === 'ty' && tyMode === 'test' && !tyTestAnswered) {
      speak(TY_WORDS[tyTestIdx % TY_WORDS.length].word);
    }
  }, [tab, tyMode, tyTestIdx, tyTestAnswered]);

  useEffect(() => {
    if (tab === 'days' && daysSub === 'test' && !dayTestAnswered) {
      speakThenSpell(DAYS[dayTestIdx % DAYS.length]);
    }
  }, [tab, daysSub, dayTestIdx, dayTestAnswered]);

  useEffect(() => {
    if (tab === 'veg' && vegSub === 'test' && !vegTestAnswered) {
      speakThenSpell(VEGETABLES[vegTestIdx % VEGETABLES.length].name);
    }
  }, [tab, vegSub, vegTestIdx, vegTestAnswered]);

  useEffect(() => {
    if (tab === 'garden' && gardenSub === 'test' && !gardenTestAnswered) {
      speakThenSpell(GARDEN[gardenTestIdx % GARDEN.length].name);
    }
  }, [tab, gardenSub, gardenTestIdx, gardenTestAnswered]);

  useEffect(() => {
    if (tab === 'kitchen' && kitchenSub === 'test' && !kitchenTestAnswered) {
      speakThenSpell(KITCHEN[kitchenTestIdx % KITCHEN.length].name);
    }
  }, [tab, kitchenSub, kitchenTestIdx, kitchenTestAnswered]);

  useEffect(() => {
    if (tab === 'occupation' && occSub === 'test' && !occTestAnswered) {
      speakThenSpell(OCCUPATIONS[occTestIdx % OCCUPATIONS.length].name);
    }
  }, [tab, occSub, occTestIdx, occTestAnswered]);

  useEffect(() => {
    if (tab === 'birds' && birdSub === 'test' && !birdTestAnswered) {
      speakThenSpell(BIRDS[birdTestIdx % BIRDS.length].name);
    }
  }, [tab, birdSub, birdTestIdx, birdTestAnswered]);

  useEffect(() => {
    if (tab === 'animals' && animalSub === 'test' && !animalTestAnswered) {
      speakThenSpell(ANIMALS[animalTestIdx % ANIMALS.length].name);
    }
  }, [tab, animalSub, animalTestIdx, animalTestAnswered]);

  useEffect(() => {
    if (tab === 'months' && monthSub === 'test' && !monthTestAnswered) {
      speakThenSpell(MONTHS_LIST[monthTestIdx % MONTHS_LIST.length]);
    }
  }, [tab, monthSub, monthTestIdx, monthTestAnswered]);
  ...
  const handleShapeTestClick = useCallback((clickedId: string) => {
  ...
  const handleColorTestClick = useCallback((clickedColor: string) => {
  ...
  const handleD3TestClick = useCallback((clickedId: string) => {
  ...
  const handleTeenTestClick = useCallback((clickedWord: string) => {
  ...
  const handleTyTestClick = useCallback((clickedWord: string) => {
  ...
  const handleDayTestClick = useCallback((clickedWord: string) => {
  ...
  const handleVegTestClick = useCallback((clickedWord: string) => {
  ...
  const handleGardenTestClick = useCallback((clickedWord: string) => {
  ...
  const handleKitchenTestClick = useCallback((clickedWord: string) => {
  ...
  const handleOccTestClick = useCallback((clickedWord: string) => {
  ...
  const handleBirdTestClick = useCallback((clickedWord: string) => {
  ...
  const handleAnimalTestClick = useCallback((clickedWord: string) => {
  ...
  const handleMonthTestClick = useCallback((clickedWord: string) => {
          ...
          <div className="hidden md:flex flex-wrap gap-1.5 border-b border-slate-200 pb-3">
            {(showAllTabs ? tabs : tabs.slice(0, 8)).map((t) => (
              <button key={t.key} type="button" onClick={() => {
                ...
                setShapeTestIdx(Math.floor(Math.random() * shapes.length)); setShapeTestAnswered(false); setShapeTestCorrect(false); setShapeTestScore(0); setShapeTestTotal(0);
                ...
                setColorTestIdx(Math.floor(Math.random() * COLORS.length)); setColorTestAnswered(false); setColorTestCorrect(false); setColorTestScore(0); setColorTestTotal(0);
                ...
                setD3TestIdx(Math.floor(Math.random() * shape3dList.length)); setD3TestAnswered(false); setD3TestCorrect(false); setD3TestScore(0); setD3TestTotal(0);
                ...
                setTeenTestIdx(Math.floor(Math.random() * TEEN_WORDS.length)); setTeenTestAnswered(false); setTeenTestCorrect(false); setTeenTestScore(0); setTeenTestTotal(0);
                ...
                setTyTestIdx(Math.floor(Math.random() * TY_WORDS.length)); setTyTestAnswered(false); setTyTestCorrect(false); setTyTestScore(0); setTyTestTotal(0);
                ...
                if (t.key === 'days') { setDaysSub('learn'); setDaysIdx(Math.floor(Math.random() * DAYS.length)); setDaysQuiz(false); setDaysAnswered(false); setDaysCorrect(false); setDaysScore(0); setDaysTotal(0); setDMemoRunning(false); setDMemoPaused(false); setDMemoLocked(false); setDaysFibResult(null); setDaysFibInput(''); setDayTestIdx(Math.floor(Math.random() * DAYS.length)); setDayTestAnswered(false); setDayTestCorrect(false); setDayTestScore(0); setDayTestTotal(0); }
                if (t.key === 'veg') { setVegSub('learn'); setVegIdx(Math.floor(Math.random() * VEGETABLES.length)); setVegQuiz(false); setVegAnswered(false); setVegCorrect(false); setVegScore(0); setVegTotal(0); setVMemoRunning(false); setVMemoPaused(false); setVMemoLocked(false); setVegFibResult(null); setVegFibInput(''); setVegTestIdx(Math.floor(Math.random() * VEGETABLES.length)); setVegTestAnswered(false); setVegTestCorrect(false); setVegTestScore(0); setVegTestTotal(0); }
                if (t.key === 'garden') { setGardenSub('learn'); setGardenIdx(Math.floor(Math.random() * GARDEN.length)); setGardenQuiz(false); setGardenAnswered(false); setGardenCorrect(false); setGardenScore(0); setGardenTotal(0); setGMemoRunning(false); setGMemoPaused(false); setGMemoLocked(false); setGardenFibResult(null); setGardenFibInput(''); setGardenTestIdx(Math.floor(Math.random() * GARDEN.length)); setGardenTestAnswered(false); setGardenTestCorrect(false); setGardenTestScore(0); setGardenTestTotal(0); }
                if (t.key === 'kitchen') { setKitchenSub('learn'); setKitchenIdx(Math.floor(Math.random() * KITCHEN.length)); setKitchenQuiz(false); setKitchenAnswered(false); setKitchenCorrect(false); setKitchenScore(0); setKitchenTotal(0); setKMemoRunning(false); setKMemoPaused(false); setKMemoLocked(false); setKitchenFibResult(null); setKitchenFibInput(''); setKitchenTestIdx(Math.floor(Math.random() * KITCHEN.length)); setKitchenTestAnswered(false); setKitchenTestCorrect(false); setKitchenTestScore(0); setKitchenTestTotal(0); }
                if (t.key === 'occupation') { setOccSub('learn'); setOccIdx(Math.floor(Math.random() * OCCUPATIONS.length)); setOccQuiz(false); setOccAnswered(false); setOccCorrect(false); setOccScore(0); setOccTotal(0); setOccMemoRunning(false); setOccMemoPaused(false); setOccMemoLocked(false); setOccFibResult(null); setOccFibInput(''); setOccTestIdx(Math.floor(Math.random() * OCCUPATIONS.length)); setOccTestAnswered(false); setOccTestCorrect(false); setOccTestScore(0); setOccTestTotal(0); }
                if (t.key === 'birds') { setBirdSub('learn'); setBirdIdx(Math.floor(Math.random() * BIRDS.length)); setBirdQuiz(false); setBirdAnswered(false); setBirdCorrect(false); setBirdScore(0); setBirdTotal(0); setBirdMemoRunning(false); setBirdMemoPaused(false); setBirdMemoLocked(false); setBirdFibResult(null); setBirdFibInput(''); setBirdTestIdx(Math.floor(Math.random() * BIRDS.length)); setBirdTestAnswered(false); setBirdTestCorrect(false); setBirdTestScore(0); setBirdTestTotal(0); }
                if (t.key === 'animals') { setAnimalSub('learn'); setAnimalIdx(Math.floor(Math.random() * ANIMALS.length)); setAnimalQuiz(false); setAnimalAnswered(false); setAnimalCorrect(false); setAnimalScore(0); setAnimalTotal(0); setAnimalMemoRunning(false); setAnimalMemoPaused(false); setAnimalMemoLocked(false); setAnimalFibResult(null); setAnimalFibInput(''); setAnimalTestIdx(Math.floor(Math.random() * ANIMALS.length)); setAnimalTestAnswered(false); setAnimalTestCorrect(false); setAnimalTestScore(0); setAnimalTestTotal(0); }
                if (t.key === 'months') { setMonthSub('learn'); setMonthIdx(Math.floor(Math.random() * MONTHS_LIST.length)); setMonthQuiz(false); setMonthAnswered(false); setMonthCorrect(false); setMonthScore(0); setMonthTotal(0); setMonthMemoRunning(false); setMonthMemoPaused(false); setMonthMemoLocked(false); setMonthFibResult(null); setMonthFibInput(''); setMonthIntroDone(false); setMonthTestIdx(Math.floor(Math.random() * MONTHS_LIST.length)); setMonthTestAnswered(false); setMonthTestCorrect(false); setMonthTestScore(0); setMonthTestTotal(0); }
        ...
        {tab === 'shapes' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setShapeSub(m); setSMemoRunning(false); setShapeAnswered(false); setShapeCorrect(false); setShapeFibResult(null); setShapeFibInput(''); setShapeTestIdx(Math.floor(Math.random() * shapes.length)); setShapeTestAnswered(false); setShapeTestCorrect(false); setShapeTestScore(0); setShapeTestTotal(0); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${shapeSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
            ...
            {shapeSub === 'test' ? (() => {
              ...
              const testOptions = [correctShape, ...shapes.filter(s => s.id !== correctShape.id).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  ...
                  {shapeTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${shapeTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {shapeTestCorrect ? '✅ Correct!' : `❌ That's a ${shapes[shapeTestIdx % shapes.length].label}`}
                    </div>
                  )}
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {shapeTestScore}/{shapeTestTotal}</div>
                </div>
              );
            })() : shapeSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'colors' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setColorSub(m); setColorAnswered(false); setColorCorrect(false); setColorMemoRunning(false); setColorMemoLocked(false); setColorFibResult(null); setColorFibInput(''); setColorTestIdx(Math.floor(Math.random() * COLORS.length)); setColorTestAnswered(false); setColorTestCorrect(false); setColorTestScore(0); setColorTestTotal(0); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${colorSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {colorSub === 'test' ? (() => {
              const colorOptions = [COLORS[colorTestIdx % COLORS.length], ...COLORS.filter((_, i) => i !== colorTestIdx % COLORS.length).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  ...
                  {colorTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${colorTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {colorTestCorrect ? '✅ Correct!' : `❌ It's ${COLORS[colorTestIdx % COLORS.length]}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-4">
                    {colorOptions.map((c) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {colorTestScore}/{colorTestTotal}</div>
                </div>
              );
            })() : colorSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'd3' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setD3Mode(m); setD3Answered(false); setD3Correct(false); setD3MemoRunning(false); setD3MemoLocked(false); setD3FibResult(null); setD3FibInput(''); setD3TestIdx(Math.floor(Math.random() * shape3dList.length)); setD3TestAnswered(false); setD3TestCorrect(false); setD3TestScore(0); setD3TestTotal(0); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${d3Mode === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {d3Mode === 'test' ? (() => {
              ...
              const testOptions = [correctShape, ...shape3dList.filter(s => s.id !== correctShape.id).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which 3D shape did you hear?</h2>
                  <p className="mt-1 text-xs text-slate-400">Listen carefully and tap the correct shape</p>
                  {d3TestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${d3TestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {d3TestCorrect ? '✅ Correct!' : `❌ That's a ${shape3dList[d3TestIdx % shape3dList.length].label}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-4">
                    {testOptions.map((shape) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {d3TestScore}/{d3TestTotal}</div>
                </div>
              );
            })() : d3Mode === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'teen' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setTeenMode(m); setTeenAnswered(false); setTeenCorrect(false); setTeenMemoRunning(false); setTeenMemoLocked(false); setTeenFibResult(null); setTeenFibInput(''); setTeenTestIdx(Math.floor(Math.random() * TEEN_WORDS.length)); setTeenTestAnswered(false); setTeenTestCorrect(false); setTeenTestScore(0); setTeenTestTotal(0); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${teenMode === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {teenMode === 'test' ? (() => {
              ...
              const testOptions = [correctWord, ...TEEN_WORDS.filter((_, i) => i !== teenTestIdx % TEEN_WORDS.length).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  ...
                  {teenTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${teenTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {teenTestCorrect ? '✅ Correct!' : `❌ It's ${correctWord.word}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {teenTestScore}/{teenTestTotal}</div>
                </div>
              );
            })() : teenMode === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'ty' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setTyMode(m); setTyAnswered(false); setTyCorrect(false); setTyMemoRunning(false); setTyMemoLocked(false); setTyFibResult(null); setTyFibInput(''); setTyTestIdx(Math.floor(Math.random() * TY_WORDS.length)); setTyTestAnswered(false); setTyTestCorrect(false); setTyTestScore(0); setTyTestTotal(0); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${tyMode === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {tyMode === 'test' ? (() => {
              ...
              const testOptions = [correctWord, ...TY_WORDS.filter((_, i) => i !== tyTestIdx % TY_WORDS.length).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which number did you hear?</h2>
                  <p className="mt-1 text-xs text-slate-400">Listen and tap the matching word</p>
                  {tyTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${tyTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {tyTestCorrect ? '✅ Correct!' : `❌ It's ${correctWord.word}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {tyTestScore}/{tyTestTotal}</div>
                </div>
              );
            })() : tyMode === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'days' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setDaysSub(m); setDaysAnswered(false); setDaysCorrect(false); setDaysQuiz(false); setDMemoRunning(false); setDMemoLocked(false); setDaysFibResult(null); setDaysFibInput(''); setDayTestIdx(Math.floor(Math.random() * DAYS.length)); setDayTestAnswered(false); setDayTestCorrect(false); setDayTestScore(0); setDayTestTotal(0); if (m === 'fib') setDaysQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${daysSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {daysSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...DAYS.filter((_, i) => i !== dayTestIdx % DAYS.length).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which word did you hear?</h2>
                  {dayTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${dayTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {dayTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {dayTestScore}/{dayTestTotal}</div>
                </div>
              );
            })() : daysSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'veg' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setVegSub(m); setVegAnswered(false); setVegCorrect(false); setVegQuiz(false); setVMemoRunning(false); setVMemoLocked(false); setVegFibResult(null); setVegFibInput(''); setVegTestIdx(Math.floor(Math.random() * VEGETABLES.length)); setVegTestAnswered(false); setVegTestCorrect(false); setVegTestScore(0); setVegTestTotal(0); if (m === 'fib') setVegQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${vegSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {vegSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...VEGETABLES.filter((_, i) => i !== vegTestIdx % VEGETABLES.length).sort(() => Math.random() - 0.5).slice(0, 3).map(v => v.name)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which vegetable did you hear?</h2>
                  {vegTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${vegTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {vegTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {vegTestScore}/{vegTestTotal}</div>
                </div>
              );
            })() : vegSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'garden' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setGardenSub(m); setGardenAnswered(false); setGardenCorrect(false); setGardenQuiz(false); setGMemoRunning(false); setGMemoLocked(false); setGardenFibResult(null); setGardenFibInput(''); setGardenTestIdx(Math.floor(Math.random() * GARDEN.length)); setGardenTestAnswered(false); setGardenTestCorrect(false); setGardenTestScore(0); setGardenTestTotal(0); if (m === 'fib') setGardenQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${gardenSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {gardenSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...GARDEN.filter((_, i) => i !== gardenTestIdx % GARDEN.length).sort(() => Math.random() - 0.5).slice(0, 3).map(v => v.name)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which garden object did you hear?</h2>
                  {gardenTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${gardenTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {gardenTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {gardenTestScore}/{gardenTestTotal}</div>
                </div>
              );
            })() : gardenSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'kitchen' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setKitchenSub(m); setKitchenAnswered(false); setKitchenCorrect(false); setKitchenQuiz(false); setKMemoRunning(false); setKMemoLocked(false); setKitchenFibResult(null); setKitchenFibInput(''); setKitchenTestIdx(Math.floor(Math.random() * KITCHEN.length)); setKitchenTestAnswered(false); setKitchenTestCorrect(false); setKitchenTestScore(0); setKitchenTestTotal(0); if (m === 'fib') setKitchenQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${kitchenSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {kitchenSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...KITCHEN.filter((_, i) => i !== kitchenTestIdx % KITCHEN.length).sort(() => Math.random() - 0.5).slice(0, 3).map(v => v.name)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which kitchen object did you hear?</h2>
                  {kitchenTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${kitchenTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {kitchenTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {kitchenTestScore}/{kitchenTestTotal}</div>
                </div>
              );
            })() : kitchenSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'occupation' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setOccSub(m); setOccAnswered(false); setOccCorrect(false); setOccQuiz(false); setOccMemoRunning(false); setOccMemoLocked(false); setOccFibResult(null); setOccFibInput(''); setOccTestIdx(Math.floor(Math.random() * OCCUPATIONS.length)); setOccTestAnswered(false); setOccTestCorrect(false); setOccTestScore(0); setOccTestTotal(0); if (m === 'fib') setOccQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${occSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {occSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...OCCUPATIONS.filter((_, i) => i !== occTestIdx % OCCUPATIONS.length).sort(() => Math.random() - 0.5).slice(0, 3).map(v => v.name)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which occupation did you hear?</h2>
                  {occTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${occTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {occTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {occTestScore}/{occTestTotal}</div>
                </div>
              );
            })() : occSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'birds' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setBirdSub(m); setBirdAnswered(false); setBirdCorrect(false); setBirdQuiz(false); setBirdMemoRunning(false); setBirdMemoLocked(false); setBirdFibResult(null); setBirdFibInput(''); setBirdTestIdx(Math.floor(Math.random() * BIRDS.length)); setBirdTestAnswered(false); setBirdTestCorrect(false); setBirdTestScore(0); setBirdTestTotal(0); if (m === 'fib') setBirdQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${birdSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {birdSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...BIRDS.filter((_, i) => i !== birdTestIdx % BIRDS.length).sort(() => Math.random() - 0.5).slice(0, 3).map(v => v.name)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which bird did you hear?</h2>
                  {birdTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${birdTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {birdTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {birdTestScore}/{birdTestTotal}</div>
                </div>
              );
            })() : birdSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'animals' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setAnimalSub(m); setAnimalAnswered(false); setAnimalCorrect(false); setAnimalQuiz(false); setAnimalMemoRunning(false); setAnimalMemoLocked(false); setAnimalFibResult(null); setAnimalFibInput(''); setAnimalTestIdx(Math.floor(Math.random() * ANIMALS.length)); setAnimalTestAnswered(false); setAnimalTestCorrect(false); setAnimalTestScore(0); setAnimalTestTotal(0); if (m === 'fib') setAnimalQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${animalSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {animalSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...ANIMALS.filter((_, i) => i !== animalTestIdx % ANIMALS.length).sort(() => Math.random() - 0.5).slice(0, 3).map(v => v.name)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which animal did you hear?</h2>
                  {animalTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${animalTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {animalTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {animalTestScore}/{animalTestTotal}</div>
                </div>
              );
            })() : animalSub === 'quiz' ? (
              <div className="text-center">
        ...
        {tab === 'months' && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex gap-2">
                {(['learn', 'quiz', 'fib', 'test'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => { setMonthSub(m); setMonthAnswered(false); setMonthCorrect(false); setMonthQuiz(false); setMonthMemoRunning(false); setMonthMemoLocked(false); setMonthFibResult(null); setMonthFibInput(''); setMonthTestIdx(Math.floor(Math.random() * MONTHS_LIST.length)); setMonthTestAnswered(false); setMonthTestCorrect(false); setMonthTestScore(0); setMonthTestTotal(0); if (m === 'fib') setMonthQuiz(true); }} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${monthSub === m ? 'bg-fuchsia-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {m === 'learn' ? '🧠 Memorize' : m === 'quiz' ? '📖 Learn' : m === 'test' ? '🎯 Quiz' : '✏️ Type It'}
                  </button>
                ))}
              </div>
            ...
            {monthSub === 'test' ? (() => {
              ...
              const testOptions = [currentWord, ...MONTHS_LIST.filter((_, i) => i !== monthTestIdx % MONTHS_LIST.length).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
              return (
                <div className="text-center">
                  <h2 className="mt-5 text-lg font-bold text-slate-900">🎯 Which month did you hear?</h2>
                  {monthTestAnswered && (
                    <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${monthTestCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {monthTestCorrect ? '✅ Correct!' : `❌ It's ${currentWord}`}
                    </div>
                  )}
                  <div className="mt-5 flex flex-wrap justify-center gap-3">
                    {testOptions.map((w) => (
                  ...
                  <div className="mt-4 text-sm text-slate-500">Score: {monthTestScore}/{monthTestTotal}</div>
                </div>
              );
            })() : monthSub === 'quiz' ? (
              <div className="text-center">
