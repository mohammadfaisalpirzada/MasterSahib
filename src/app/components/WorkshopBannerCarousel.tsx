...
export default function WorkshopBannerCarousel() {
  ...
  return (
    <section aria-label="Latest workshops and announcements" className="bg-[#f4f7fb] px-3 pb-2 pt-4 sm:px-6 sm:pt-6">
      <div className="group relative mx-auto max-w-7xl overflow-hidden rounded-[1.5rem] bg-slate-950 shadow-[0_22px_70px_-30px_rgba(15,23,42,0.7)] sm:rounded-[2rem]">
        {loading ? (
          ...
          <>
            ...
            {banners.length > 1 ? (
              <>
                <button type="button" onClick={() => showSlide(activeIndex - 1)} aria-label="Previous banner" className="absolute left-3 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/30 bg-black/35 text-2xl text-white backdrop-blur transition hover:bg-black/60 sm:left-5">‹</button>
                <button type="button" onClick={() => showSlide(activeIndex + 1)} aria-label="Next banner" className="absolute right-3 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/30 bg-black/35 text-2xl text-white backdrop-blur transition hover:bg-black/60 sm:right-5">›</button>
